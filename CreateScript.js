...
const generateOffice = (settingsString) => {
	...
	if (settings.length == 10) {
		...
		if (!settings[0]) config.push("<ExcludeApp ID=\"Access\" />");
		if (!settings[1]) config.push("<ExcludeApp ID=\"Excel\" />");
		if (!settings[2]) config.push("<ExcludeApp ID=\"Groove\" />");
		if (!settings[3]) config.push("<ExcludeApp ID=\"Lync\" />");
		if (!settings[4]) config.push("<ExcludeApp ID=\"OneDrive\" />");
		if (!settings[5]) config.push("<ExcludeApp ID=\"OneNote\" />");
		if (!settings[6]) config.push("<ExcludeApp ID=\"Outlook\" />");
		if (!settings[7]) config.push("<ExcludeApp ID=\"PowerPoint\" />");
		if (!settings[8]) config.push("<ExcludeApp ID=\"Publisher\" />");
		if (!settings[9]) config.push("<ExcludeApp ID=\"Word\" />");
		config.push("</Product>");
		config.push("</Add>");
